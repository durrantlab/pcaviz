...
def get_PCA_trajectory(traj, selection, cum_var):
    ...
    data_json = []
    # loops over atom information turns the atom name, residue name, and resid into a string

    for a in atomgroup:
        at_name = str(a.name)
        res_name = str(a.resname)
        res_id = str(a.resid)
    # if statement will check to see if residue id is present if it's not present it will add
    # the residue id, residue name, and the residue's atoms 
...
if __name__ == '__main__':
    ...
    vect_components, PCA_coeff, coords_avg_atoms, data_json_info = get_PCA_trajectory(traj,
                                                                      params['selection'],
                                                                   params['cum_var'])
