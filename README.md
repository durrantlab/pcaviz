...
* Create a compressed JSON file from a topology (1J8K_example.psf) and a
...
* PDB files can also contain multiple frames. In this case, the same file
...
* By default, PCAViz includes only the backbone atoms in the output. These
...
* Striding the trajectory frames can reduce file sizes. PCAViz will
...
* PCAViz allows users to control the compression settings. Two settings are
...
* To find the ideal --cum_var and --precision parameters, you may wish to
...
* By default, PCAViz saves the compressed JSON file to the same directory
...
* For debugging purposes, PCAViz also includes an option to test whether the
